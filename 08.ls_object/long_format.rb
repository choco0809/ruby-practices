...
class LongFormat
  ...
  def fetch_files_permission(files_mode)
    files_mode[-3, 3].each_char.each_with_object([]) do |file_mode, permission_type_strings|
      permission_type_strings << permission_type_list(file_mode.to_i)
    end
  end

  def fetch_files_type(files_mode)
  ...
  def fetch_files_info(files)
    files.map do |file|
      ...
      file_mode = stat_file.mode.to_s(8)
      permission_strings = fetch_files_permission(file_mode)
      files_type = fetch_files_type(file_mode)
      file_mode_string = files_type + permission_strings.join
      {
        file_name: file,
